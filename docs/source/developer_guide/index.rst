...
``aiida-jutools`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-jutools`` repository (preferably using ``aiida-jutools`` as the project name)

The documentation is now available at `aiida-jutools.readthedocs.io <http://aiida-jutools.readthedocs.io/>`_.
...
.. note::
   ...
   When updating the plugin package to a new version, remember to update the version number both in ``setup.json`` and ``aiida_jutools/__init__.py``.
