...
def plot_convergence_scatter2(frame, frame2, iteration=None, print_prozent=True, prozent=0.0, saveas='convergence_scf_oqmd3.pdf'):
    ...
    set_plot_defaults(use_axis_fromatter=False, figsize=(16, 9), labelfonstsize=21, ticklabelsize=20)
    ...
    ax3 = multi_scatter_plot(x3,y3,markersize, xlabel='Charge distance [me/bohr^3]', ylabel=u'Energy diff [htr]',
                            title='', scale=['log', 'log'], legend=False,
                            #xticks=xticks,
                            color=markercolor,
                            limits=[[0.0000001, 1100],[0.00000000001, 1100]],
                            saveas='convergence_scf_oqmd', alpha=0.5, marker='o', label='spin', axis=ax3)#markerstyle)

    rectangle2 = mppatches.Patch(color='r', alpha=0.6, label='Collinear magnetism')
    rectangle1 = mppatches.Patch(color='b', alpha=0.6, label='Non magnetic')
    circle1 = mlines.Line2D(range(1), range(1), linestyle='', color='k', alpha=0.3, marker='o', markerfacecolor='k', markersize=5.0, label='< 20 Iterations')
    circle2 = mlines.Line2D(range(1), range(1), linestyle='', color='k', alpha=0.3, marker='o', markerfacecolor='k', markersize=10.0, label='<100 Iterations')
    circle3 = mlines.Line2D(range(1), range(1), linestyle='', color='k', alpha=0.3, marker='o', markerfacecolor='k', markersize=15.0, label='=240 Iterations')
    convl = mlines.Line2D(range(1), range(1), linestyle='--', color='k', alpha=1.0, marker='', label='Convergence criterion')

    handles = [circle1, circle2, circle3, rectangle1, rectangle2, convl]
    legends_defaults = {'bbox_to_anchor' : (0.05, 0.97), 'fontsize' : 17,
                                'linewidth' : 3.0, 'borderaxespad' : 0 , 'loc' : 2,
                                'fancybox' : True, 'framealpha' : 1.0} #'title' : 'Legend',
    loptions = legends_defaults.copy()
    linewidth = loptions.pop('linewidth', 1.5)
    leg = ax3.legend(handles=handles, **loptions)#bbox_to_anchor=loptions['anchor'],loc=loptions['loc'], title=legend_title, borderaxespad=0., fancybox=True)
    leg.get_frame().set_linewidth(linewidth)
    ax3.axvline(ymin=0, ymax=1.0, x=0.000005, linewidth=2.0, linestyle='--', color='k')
    ax3 = single_scatterplot(x2,y2, u"Charge distance [me/a$_0$$^3$]",
                             ylabel=u'Energy difference [htr]',#last $\Delta$ E [htr]',
                             marker='', title='', plotlabel='', axis=ax3)
    ...
    histcolors=['r', 'b']
    ax1.axvline(ymin=0, ymax=1.0, x=0.000005, linewidth=2.0, linestyle='--', color='k')
    ax1, n, bins2, patches= histogram(x,bins=bins, legend=False,
                                      xlabel='',#charge distance [me/bohr^3]',
                            ylabel=u'N', histtype='barstacked',
                            title='', cumulative=False, color=histcolors,
                            limits=[[0.0000001, 1100],[0, 1200]],
                            saveas='convergence_scf_oqmd3', alpha=0.6, axis=ax1,return_hist_output=True)#markerstyle)
    ...
    ax1.text(0.5, 0.7, r'$N_{tot}$ =           +', #+ #str(len(difference_charge_spin[0])) + r'+'+ str(len(difference_charge_spin[1])),
             #r'\textcolor{blue}{{}}'.format(str(len(difference_charge_spin[0]))) + r'+'+ r'\textcolor{red}{{}}'.format(str(len(difference_charge_spin[1]))),
            horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes, fontsize=22)
    ax1.text(0.54, 0.7, '{}'.format(str(len(difference_charge_spin[0]))), color='red',
             horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes, fontsize=22)
    ax1.text(0.64, 0.7, '{}'.format(str(len(difference_charge_spin[1]))), color='blue',
             horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes, fontsize=22)
    if iteration is not None:
        ax1.text(0.9, 0.7, u'Iter = ' + str(iteration),
            horizontalalignment='center',
            verticalalignment='center', transform=ax1.transAxes, fontsize=22)
    ...
    if print_prozent: # maybe to slow
        ...
        ax1.text(0.055, 0.7, u'{:3.0f}%'.format(prozent),
            horizontalalignment='center',
            verticalalignment='center', transform=ax1.transAxes, fontsize=22)
        ax1.text(0.25, 0.7, u'{:3.0f}%'.format(100-prozent),
            horizontalalignment='center',
            verticalalignment='center', transform=ax1.transAxes, fontsize=22)
    ...
    ax4, n, bins2, patches = histogram(x4,bins=bins1, legend=False,
                            xlabel=u'N',#charge distance [me/bohr^3]',
                            ylabel=u'', histtype='barstacked', orientation='horizontal',
                            title='', cumulative=False, color=histcolors,
                            limits=[[0, 1200],[0.00000000001, 1100]],
                            saveas='convergence_scf_oqmd3', alpha=0.6, axis=ax4,return_hist_output=True)#markerstyle)
