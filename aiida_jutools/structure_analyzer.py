...
def compare_hall_numbers(sg, cifspg):
    ...
    spacegroup_to_hall_number = [
        1, 2, 3, 6, 9, 18, 21, 30, 39, 57, 60, 63, 72, 81, 90, 108, 109, 112,
        115, 116, 119, 122, 123, 124, 125, 128, 134, 137, 143, 149, 155, 161,
        164, 170, 173, 176, 182, 185, 191, 197, 203, 209, 212, 215, 218, 221,
        227, 228, 230, 233, 239, 245, 251, 257, 263, 266, 269, 275, 278, 284,
        290, 292, 298, 304, 310, 313, 316, 322, 334, 335, 337, 338, 341, 343,
        349, 350, 351, 352, 353, 354, 355, 356, 357, 358, 359, 361, 363, 364,
        366, 367, 368, 369, 370, 371, 372, 373, 374, 375, 376, 377, 378, 379,
        380, 381, 382, 383, 384, 385, 386, 387, 388, 389, 390, 391, 392, 393,
        394, 395, 396, 397, 398, 399, 400, 401, 402, 404, 406, 407, 408, 410,
        412, 413, 414, 416, 418, 419, 420, 422, 424, 425, 426, 428, 430, 431,
        432, 433, 435, 436, 438, 439, 440, 441, 442, 443, 444, 446, 447, 448,
        449, 450, 452, 454, 455, 456, 457, 458, 460, 462, 463, 464, 465, 466,
        467, 468, 469, 470, 471, 472, 473, 474, 475, 476, 477, 478, 479, 480,
        481, 482, 483, 484, 485, 486, 487, 488, 489, 490, 491, 492, 493, 494,
        495, 497, 498, 500, 501, 502, 503, 504, 505, 506, 507, 508, 509, 510,
        511, 512, 513, 514, 515, 516, 517, 518, 520, 521, 523, 524, 525, 527,
        529, 530, 531
    ]
    ...
    for hall_num in range(hall_num0, hall_num1):
        ...
        rot1 = symops['rotations']
        shaper1 = numpy.shape(rot1)
        rot2 = cifspg['rotations']
        shaper2 = numpy.shape(rot2)

        trans1 = symops['translations']
        shapet1 = numpy.shape(trans1)
        trans2 = cifspg['translations']
        shapet2 = numpy.shape(trans2)

        if (shaper1[0] != shaper2[0]) or (shapet1[0] != shapet2[0]) or (
                shaper1[0] != shapet1[0]):
            continue
        ...
        for idx1 in range(0, len(trans1)):
            ...
            for idx2 in range(0, len(trans2)):
                if (matched[idx2] == 0) and numpy.allclose(
                        rot1[idx1], rot2[idx2]) and numpy.allclose(
                            trans1[idx1], trans2[idx2]):
                    matched[idx2] = 1
                    idxmatch = True
            if idxmatch == False:
                break
...
def check_setdif(mydict, key0, key1, strict, setdiflist):
    ...
    if len(myset) != 1:
        if strict:
            print(prompt + CWR + ' !!! inconsistent info[' + "'" + key0 + "'" +
                  '][' + "'" + key1 + "']" + CEND + ' : [ ' + ', '.join(
                      str("'" + k + "' = ") + str(CWR + v + CEND)
                      for k, v in sorted(mydict[key0][key1].items())) + ' ]' +
                  CEND)
            setdiflist[0] = False
        else:
            setdiflist[1] = True
            setdiflist[2] += " ; '" + key1 + "' : [ " + ', '.join(
                str("'" + k + "' = ") + str("'" + v + "'")
                for k, v in sorted(mydict[key0][key1].items())) + ' ]'
...
def cif_check(cif_read, cif_data, spgl):
    ...
    if len(match) == 1:
        ...
        info['check'] = 1
        info[
            'message'] = 'OK ... ' + pr_m4s + ' one matching Hall number found'
    else:
        if len(match) > 1:
            info['check'] = 2
            info[
        ...
        if len(match2) == 1:
            info['check'] = -1
            info[
                'message'] = 'Maybe OK ... ' + pr_m4s2 + ' one matching Hall number in different space group found'

    alright = True
    difdet = False
    prdifdet = ''
    setdiflist = [
        True, False, CWR + ' !!! original and default choices differ' + CEND
    ]
    for key0 in info0:
        info[key0] = {}
        for key1, val1 in info1.items():
            ...
            if key0 == 'sg' and key1 == 'number':
                info[key0][key1]['read'] = str(
                    cif_read.get(
                        cif_read.keys()[0]).get('_symmetry_Int_Tables_number'))
                info[key0][key1]['data'] = str(
                    cif_data.get_spacegroup_numbers()[0])
            ...
            if key0 == 'sg' and key1 == 'international_full':
                info[key0][key1]['read'] = cif_read.get(
                    cif_read.keys()[0]).get('_symmetry_space_group_name_h-m')
                info[key0][key1]['spgo'] = str(sg_spgo.get(key1))
                info[key0][key1]['spgd'] = str(sg_spgd.get(key1))
...
def construct_symmetry_dict(dim, sgn, sgs, pgs):
    ...
    if dim == [3, 3]:
        ...
        sgn2cs = [[1, 2, 'triclinic'], [3, 15, 'monoclinic'],
                  [16, 74, 'orthorhombic'], [75, 142, 'tetragonal'],
                  [143, 167, 'trigonal'], [168, 194, 'hexagonal'],
                  [195, 230, 'cubic']]
        sgn2cf = [[1, 2, 'a'], [3, 15, 'm'], [16, 74, 'o'], [75, 142, 't'],
                  [143, 194, 'h'], [195, 230, 'c']]
        sgn2ic = [[2, 2], [10, 15], [47, 74], [83, 88], [123, 142], [147, 148],
                  [162, 167], [175, 176], [191, 194], [200, 206], [221, 230]]
        ltt = [True, True]
        ltf = [True, False]
        lft = [False, True]
        lff = [False, False]
        ...
        if crystal_family == 'm' and lattice_centering != 'P':
            lattice_centering = 'S'
        if crystal_family == 'o' and (lattice_centering == 'A'
                                      or lattice_centering == 'B'
                                      or lattice_centering == 'C'):
            lattice_centering = 'S'
        ...
        sdict[
            'point_group_center_of_inversion'] = point_group_center_of_inversion
...
def determine_wyckrep(pymg_dict, spgl, symmetry_dict):
    ...
    if nsites_pymg != nsites_spgl:
        xproto['status'] = 'ERROR'
        xproto['message'] = 'nsites_pymg (' + str(
            nsites_pymg) + ') != nsites_spgl (' + str(nsites_spgl) + ')'
    ...
    for idx, repidx in enumerate(repidxlist):
        eqdict[repidx] = {}
        eqdict[repidx][
            'idx_eq'] = idx  # index of representative site in eqdict
        eqdict[repidx]['sites_label'] = pymg_dict['sites'][repidx][
            'label']  # site labels, copy from pymg_dict
        eqdict[repidx]['sites_species'] = pymg_dict['sites'][repidx][
            'species']  # site species, copy from pymg_dict
        eqdict[repidx]['sites_conc'] = round(
            sum([x['occu'] for x in eqdict[repidx]['sites_species']]),
            roundtol)  # sum of occupations over species at representative site
        eqdict[repidx]['mult_nom'] = (spgl['equivalent_atoms'] == repidx).sum(
        )  # nominal multiplicity = wyckoff multiplicity
        eqdict[repidx]['mult_prox'] = proximate_multiplicity(
            eqdict[repidx]['mult_nom'],
            eqdict[repidx]['sites_conc'])  # proximate multiplicity
        eqdict[repidx]['list_eleocc'] = [
            [eleocc['element'], eleocc['occu']]
            for eleocc in eqdict[repidx]['sites_species']
        ]  # list of [element, occupation] pairs to be used in wyck
        eqdict[repidx]['list_eleocc'] = sorted(eqdict[repidx]['list_eleocc'])
        # setting up representative label eqdict_label
        if eqdict[repidx][
                'mult_prox'] > 0:  # reasonably high (> 0) occupancy of the Wyckoff positions
            mult_max_eq = max([
                list(spgl['equivalent_atoms']).count(x) for x in repidxlist if
                pymg_dict['sites'][x]['label'] == eqdict[repidx]['sites_label']
            ])
            mult_ind = [
                proximate_multiplicity(mult_max_eq, x[1])
                for x in eqdict[repidx]['list_eleocc']
            ]  # individual proximate multiplicity
            if all(mi == 0 for mi in mult_ind):
                ...
                if sum(mi > 0 for mi in mult_ind) == 1:
                    eqdict_label = ' '.join([
                        eqdict[repidx]['list_eleocc'][y][0]
                        for y, x in enumerate(mult_ind) if x > 0
                    ])
                else:
                    eqdict_label = ' '.join([
                        eqdict[repidx]['list_eleocc'][y][0] + ':' +
                        str(eqdict[repidx]['list_eleocc'][y][1])
                        for y, x in enumerate(mult_ind) if x > 0
                    ])
        else:
            eqdict_label = 'X'
        # assigning group index to representative site, based on eqdict_label
        label_sum_not_none = sum(
            x != None
            for x in list(set([x.get('label') for x in eqdict.values()])))
        label_match = list(
            set([
                x.get('idx_grp') for x in eqdict.values()
                if x.get('label') == eqdict_label
            ]))
        if label_match == []:
    ...
    rspg = [
        x[1] for x in sorted(
            zip(spgl['std_mapping_to_primitive'].tolist(),
                spgl['std_positions'].tolist()))[:]
    ]
    ...
    for idx, site in enumerate(pymg_dict['sites']):
        ...
        if idx == equa:
            ...
            wyck[sidx][2].append([
                wycklabel, wyckmult, [None, None, None], wyckocc, wyckmo,
                eqdict[equa]['list_eleocc']
            ])
        ...
        if len(deg3rwyck) > len(deg3rabc):
            ...
            if len(deg2rwyck) > len(deg2rabc):
                ...
                arwyck2 = abs(round(repwyck[2], roundtol))
                arabc2 = abs(round(rabc[2], roundtol))
                if arabc2 < arwyck2:
                    repwyck = rabc
                elif arabc2 == arwyck2:
                    arwyck1 = abs(round(repwyck[1], roundtol))
                    arabc1 = abs(round(rabc[1], roundtol))
                    if arabc1 < arwyck1:
                        repwyck = rabc
                    elif arabc1 == arwyck1:
                        arwyck0 = abs(round(repwyck[0], roundtol))
                        arabc0 = abs(round(rabc[0], roundtol))
                        if arabc0 < arwyck0:
                            repwyck = rabc
        wyck[sidx][2][widx][2] = [
            round(repwyck[x], roundtol) for x in range(0, 3)
        ]
    ...
    cfuzid = max(
        [x[1][0][0] for x in wycksort[:] if x[1][0][0] == int(x[1][0][0])]
        or [0])
    cfuzre = max(
        [x[1][0][1] for x in wycksort[:] if x[1][0][1] == int(x[1][0][1])]
        or [0])

    for idx, wyckele in enumerate(wyck):
        # sort wyckoff positions alphabetically by their labels within group
        wycksort[idx][2] = sorted(wyckele[2],
                                  key=lambda a: (caseswap(a[0]), a[2][::-1]))
        if wycksort[idx][3] != 'X':
            ...
            wycksort[idx][0] = '~'
        if cfuzid != 0 and wycksort[idx][1][0][0] != 0 and wycksort[idx][1][0][
                0] == int(wycksort[idx][1][0][0]):
            cfuzid = int(gcd(cfuzid, wycksort[idx][1][0][0]))
        if cfuzre != 0 and wycksort[idx][1][0][1] != 0 and wycksort[idx][1][0][
                1] == int(wycksort[idx][1][0][1]):
    ...
    wycksort = sorted(wycksort,
                      key=lambda a: (caseswap(a[:][0]), a[:][2][0][2][::-1]))
    ...
    supersimple_stoich = ''
    xproto[
        'nrw'] = ''  # name constructed from letters and their sequence indices of representative Wyckoff positions
    idxw = 0
    nsites_occ = 0
    for idx, wyckele in enumerate(wycksort):
        if wycksort[idx][0] != '~':
            wycksort[idx][0] = chr(
                idx + 65)  # replace temporary label by uppercase letter
            simple_stoich.append(
                wycksort[idx][0] +
                str(wycksort[idx][1][0][0]))  # append label with multiplicity
            supersimple_stoich = supersimple_stoich + wycksort[idx][
                0]  # append label and its multiplicity if it is > 1
            if wycksort[idx][1][0][0] != 1:
                supersimple_stoich = supersimple_stoich + str(
                    wycksort[idx][1][0][0])
    ...
    pearson = symmetry_dict['crystal_family'] + symmetry_dict[
        'lattice_centering'] + str(nsites_occ)
    xproto['nprot'] = '_'.join(simple_stoich) + '__' + pearson + '__' + str(
        spgl['number'])  # prototype name
    ...
    if len(zset_nom) != 1 or len(zset_prox) != 1:
        xproto['status'] = 'ERROR'
        xproto['message'] = 'zset_nom, zset_prox = (' + str(
            zset_nom) + ', ' + str(zset_prox) + ')'
        return (xproto, labels)

    # 'occ_structure' - occupation in nested lists, base for next entries
    occ_structure = [[[[[[z[0], z[1]] for z in y[5]], y[1]]
                       for y in x[2]], x[0]] for x in wycksort]
    occ_flat = sorted(
        sum([[[z[0], z[1] * y[1]] for z in y[0]]
             for y in sum([x[0] for x in occ_structure], [])], []))
    occ_element_alpha = [[
        ele, round(sum(y[1] for y in occ_flat if y[0] == ele), roundtom)
    ] for ele in sorted(list(set([x[0] for x in occ_flat])))]
    occ_element_atnum = sorted(occ_element_alpha,
                               key=lambda a: (ptable[a[0]][0]))
    occ_element_eniupac = sorted(occ_element_alpha,
                                 key=lambda a: (ptable[a[0]][3]))
    occ_element_gp = sorted(occ_element_alpha,
                            key=lambda a: (ptable[a[0]][2], -ptable[a[0]][1]))
    occ_element_pg = sorted(occ_element_alpha,
                            key=lambda a: (-ptable[a[0]][1], ptable[a[0]][2]))
    # 'wyck_*' entries with general stoichiometry including Wyckoff letters
    stoich_wyck_all = ' '.join([
        x[0] + str(x[1][0][0]) + '[' + ''.join([y[0] for y in x[2]]) + ']'
        for x in wycksort
    ])
    stoich_wyck_sum = ' '.join([
        x[0] + str(x[1][0][0]) + '[' + '-'.join(
            sorted([
                k + '*' + str(v) for k, v in dict(
                    zip([str(y[1]) + y[0]
                         for y in x[2]], [[y[0] for y in x[2]].count(i)
                                          for i in [y[0]
                                                    for y in x[2]]])).items()
            ],
    ...
    xproto['stoich'] = {}
    xproto['stoich']['occ_element_full'] = ' '.join([
        '{' + x[0] + '}{' + str(x[1]).rstrip('0').rstrip('.') + '}'
        for x in occ_element_alpha
    ])
    xproto['stoich']['occ_element_rat'] = ' '.join([
        '{' + x[0] + '}{' +
        str(round(1. * x[1] / zset_nom[0], roundtos)).rstrip('0').rstrip('.') +
        '}' for x in occ_element_alpha
    ])
    xproto['stoich']['occ_element_rednom'] = ' '.join([
        '{' + x[0] + '}{' + str(round(1. * x[1] / wocc[0], roundtom)) + '}'
        for x in occ_element_alpha
    ])
    xproto['stoich']['occ_element_redreal'] = ' '.join([
        '{' + x[0] + '}{' + str(round(1. * x[1] / wocc[2], roundtom)) + '}'
        for x in occ_element_alpha
    ])
    xproto['stoich']['formula_alpha'] = ' '.join([
        x[0] +
        str(round(1. * x[1] / zset_nom[0], roundtos)).rstrip('0').rstrip('.')
        for x in occ_element_alpha
    ])
    xproto['stoich']['formula_atnum'] = ' '.join([
        x[0] +
        str(round(1. * x[1] / zset_nom[0], roundtos)).rstrip('0').rstrip('.')
        for x in occ_element_atnum
    ])
    xproto['stoich']['formula_eniupac'] = ' '.join([
        x[0] +
        str(round(1. * x[1] / zset_nom[0], roundtos)).rstrip('0').rstrip('.')
        for x in occ_element_eniupac
    ])
    xproto['stoich']['formula_gp'] = ' '.join([
        x[0] +
        str(round(1. * x[1] / zset_nom[0], roundtos)).rstrip('0').rstrip('.')
        for x in occ_element_gp
    ])
    xproto['stoich']['formula_pg'] = ' '.join([
        x[0] +
        str(round(1. * x[1] / zset_nom[0], roundtos)).rstrip('0').rstrip('.')
        for x in occ_element_pg
    ])
    ...
    xproto['stoich']['form_nom_rat'] = [
        '   '.join([
            x[1] + ':[' + '  '.join([
                '(' +
                ' '.join(['{' + z[0] + '}{' + str(z[1]) + '}' for z in y[0]]) +
                ')(' + str(round(1. * y[1] / zset_nom[0],
                                 roundtos)).rstrip('0').rstrip('.') + ')'
                for y in x[0]
            ]) + ']' for x in occ_structure
        ]), zset_nom[0]
    ]
    xproto['stoich']['form_nom_ratexp'] = [
        ' '.join([
            x[1] + ':' + ''.join([
                '(' + ' '.join([
                    z[0] + '_' + str(round(z[1] * y[1] / zset_nom[0],
                                           roundtos)).rstrip('0').rstrip('.')
                    for z in y[0]
                ]) + ')' for y in x[0]
            ]) for x in occ_structure
    ...
    xproto['stoich']['form_nom_red'] = [
        '   '.join([
            x[1] + ':[' + '  '.join([
                '(' +
                ' '.join(['{' + z[0] + '}{' + str(z[1]) + '}' for z in y[0]]) +
                ')(' + str(round(1. * y[1] / wocc[0], roundtom)) + ')'
                for y in x[0]
            ]) + ']' for x in occ_structure
        ]),
    ...
    if wocc[2] != wocc[0]:
        xproto['stoich']['form_real_full'] = [
            '   '.join([
                x[1] + ':[' + '  '.join([
                    '(' + ' '.join([
                        '{' + z[0] + '}{' + str(round(z[1] * y[1], roundtom)) +
                        '}' for z in y[0]
                    ]) + ')' for y in x[0]
                ]) + ']' for x in occ_structure
            ]), 1
        ...
        xproto['stoich']['form_real_rednom'] = [
            '   '.join([
                x[1] + ':[' + '  '.join([
                    '(' + ' '.join([
                        '{' + z[0] + '}{' +
                        str(round(z[1] * y[1] / wocc[0], roundtom)) + '}'
                        for z in y[0]
                    ]) + ')' for y in x[0]
                ]) + ']' for x in occ_structure
            ]),
        ...
        xproto['stoich']['form_real_redreal'] = [
            '   '.join([
                x[1] + ':[' + '  '.join([
                    '(' + ' '.join([
                        '{' + z[0] + '}{' +
                        str(round(z[1] * y[1] / wocc[2], roundtom)) + '}'
                        for z in y[0]
                    ]) + ')' for y in x[0]
                ]) + ']' for x in occ_structure
            ]),
    ...
    labels['symmetry_v'] = ' '.join([
        ...
        ')', '[' + str(symmetry_dict['[system,lattice] dimension'][0]) + ',' +
        str(symmetry_dict['[system,lattice] dimension'][1]) + ']'
    ])
    labels['structure_v'] = pearson + ' : ' + 'Z=' + str(
        zset_nom[0]) + ' : ' + stoich_wyck_sum
...
def proximate_multiplicity(mult, conc):
    ...
    if conc < 0. or conc > 1.0 or mult < 1:
        return
    divlist = [
        x for x in range(1, mult + 1) if 1.0 * mult / x == int(1.0 * mult / x)
    ]
        ...
        for vidx, vproto in enumerate(iproto):
            ...
            if all(x == True for x in match[vidx]['items'].values()):
                ...
                for sidx in range(0, nspec):
                    vspec = str(vproto['repwyck'][sidx][0]) + str(
                        vproto['repwyck'][sidx][1])
                    xspec = str(xproto['repwyck'][sidx][0]) + str(
                        xproto['repwyck'][sidx][1][0][0])
                    ...
                    if vspec == xspec:
                        ...
                        for widx in range(0, len(proto_wlm)):
                            match[vidx]['occupied']['wyckpos'][pidx] = [
                            ...
                            for ridx in range(0, 3):
                                ...
                                if isinstance(vr, (int, float)):
                                    if abs(vr - xr) <= tolerance:
                                        match[vidx]['occupied']['wyckpos'][
                                            pidx][ridx + 1] = True
                                # checking match for coordinates with allowed range
                                if isinstance(vr, str):
                                    par_vr = vproto['params']['repwyck'].get(
                                        vr)
                                    if xr >= par_vr[0] and xr <= par_vr[1]:
                                        match[vidx]['occupied']['wyckpos'][
                                            pidx][ridx + 1] = True
                            # matching: concentration
                            vc = vproto['params']['repwyck'].get(
                                str(vwyck[0]) + str(pidx) + 'c')
                            if vc and isinstance(vc[0],
                        ...
                        match[vidx][
                            '_message'] = 'species do not match: vspec = ' + vspec + ' xspec = ' + xspec
                        return match
                ...
                match[vidx]['items']['lattice'] = all(
                    x == True for x in match[vidx]['lattice'])
                match[vidx]['items']['occupied_species'] = all(
                    x == True for x in match[vidx]['occupied']['species'])
                match[vidx]['items']['occupied_wyckpos'] = all([
                    x == True for sublist in match[vidx]['occupied']['wyckpos']
                    for x in sublist
                ])
                ...
                if vproto.get('vacant') != None:
                    ...
                    for vacidx, vacelem in enumerate(vproto['vacant']):
                        ...
                        vacpardict = {}
                        for vacpark, vacparv in vproto['params']['vacant'][
                                vacidx].items():
                        ...
                        for vacwyckidx in range(
                                0, len(match[vidx]['vacant'][vacidx][2])):
                            for ridx in range(0, 3):
                                if isinstance(
                                        match[vidx]['vacant'][vacidx][2]
                                    [vacwyckidx][2][ridx], str):
                                    match[vidx]['vacant'][vacidx][2][
                                        vacwyckidx][2][ridx] = eval(
                                            match[vidx]['vacant'][vacidx][2]
                                            [vacwyckidx][2][ridx])
...
def construct_pymg_kkr(pymg, spgo, prototype):
    ...
    for (iproto, proto) in enumerate(prototype['match']):
        if proto['_status'] == True:
            ...
            protoid = ' ) ( '.join([
                '( var: ' + varstr, 'lnl: ' + proto['id_names']['lnl'][0][0],
                'pnl: ' + ' , '.join(proto['id_names']['pnl']) + ' )'
            ])
            for (ivac, vac) in enumerate(proto['vacant']):
                ...
                for (iwv, wyckvac) in enumerate(vac[2]):
                    if not wyckvac in vacrep:
                        ...
                        wvl[iwv] = []
                        for (tran, rot) in zip(spgo['translations'],
                                               spgo['rotations']):
                            wvl[iwv].append(
                                numpy.around(
                                    (rot.dot(posvac) + tran), decimals=12) % 1)
                        wvl[iwv] = numpy.unique(numpy.asarray(wvl[iwv]),
                                                axis=0).tolist()
                        if len(wvl[iwv]) != wyckvac[1]:
                            multmatch[iwv] = False
                    else:
                        print(prompt + '    ' + str(iproto) + ' ' + str(ivac) +
                              ' vacant site ' + str(wyckvac) +
                              ' found to be partially occupied, skipping ...')
                if all(multmatch):
                    status = protoid + ' + alt. ' + str(
                        ivac) + ' :: star of wyckoff positions ' + str(
                            [len(x) for x in wvl])
                    vacar.append([status, wvl])
                    okar.append(True)
                else:
                    status = protoid + ' + alt. ' + str(
                        iwv) + ' :: ERROR in star of wyckoff positions ' + str(
                            [len(x) for x in wvl])
                    okar.append(False)
                    print(prompt + '    ' + status)
    ...
    for (ix, x) in enumerate(vacar):
        ...
        if len(x[1]) > 0:
            for iy, y in enumerate(x[1]):
                ...
                for z in y:
                    ysim.append([
                        nsimplify(w) if len(str(nsimplify(w))) <= 3 else w
                        for w in z
                    ])
                print(prompt + '        ' + 'v' + str(iy) + ' : ' +
                      ' '.join([str(z).replace(' ', '') for z in ysim]))
    if len(vacar) > 1:
        ans = input(prompt +
                    '    Choose index of one of the listed alternatives: ')
    ...
    for x in vacitem:
        for pos in x:
            idx += 1
            pymg_kkr.append({'X': 1.0},
                            pos,
                            validate_proximity=True,
                            properties={
                                **pymgkind,
                                **{
                                    'sorting_index': last_sorting_index + idx
                                }
                            })
...
def get_sis(path):
    ...
    if len(lf) == 2 and lf[0].isalpha() and lf[1].isdigit():
        ...
        sis = lf[0] + ' ' + datetime.fromtimestamp(
            os.path.getctime(path)).strftime('%Y%m%d-%H:%M:%S.%f')
...
def determine_kkr_parameters(conv, prim):
    ...
    distlist = sorted(
        list(set([round(y[1] + 0.5e-8, roundtol) for x in nnlist for y in x])))
    ...
    for dist in distlist:
        nnlistcount = [
            len(x)
            for x in pymg.get_all_neighbors_py(round(dist + 0.5e-8, roundtol))
        ]
        ...
        if nnlistcountmin > oldmin:
            ...
            nvd.append([
                round(dist + 0.5e-8, roundtol), nnlistcountmin,
                max(nnlistcount),
                sum(nnlistcount), rinalatbasis, rsimple
            ])
...
def analyze_symmetry(dd):
    ...
    if fmt == 'cif':
        ...
        if float(
                max(
                    cif_read.get(cif_read.keys()[0]).get(
                        '_atom_site_occupancy')).split('(')[0]) > 1:
            print(prompt + CWR +
                  '!!! Site occupation larger than 1 was found !!!' + CEND)
            return output
        ...
        astr_init = cif_data.get_structure(
        )  # conventional unit cell aiida StructureData
        pymg_init = astr_init.get_pymatgen(
        )  # conventional unit cell pymatgen structure
        ...
        (lattice, positions, numbers) = construct_spglib_input_from_pymatgen(
            pymg_init)  # (lattice,positions,numbers) input for spglib
        spgd = spglib.get_symmetry_dataset(
            (lattice, positions, numbers)
        )  # spglib symmetry dataset corresponding to pymg_init and astr_init
        check_cif = cif_check(
            cif_read, cif_data, spgd
        )  # check_cif dictionary contains some cross-checks about symmetry and structure

        if check_cif['check'] == 1:  # if check is successful
            # original choice/setting is used to get spgo_init spgilb symmetry dataset
            spgo_init = spglib.get_symmetry_dataset(
                (lattice, positions, numbers),
                hall_number=int(check_cif['sg']['hall_number']['spgo']))
            ...
            system['components'][0]['objects'][0]['character'] = [
                'crystalline'
            ]
            if astr_init.is_alloy:
                system['components'][0]['objects'][0]['character'].append(
                    'alloyed')
            ...
            system['components'][0]['objects_boundary_conditions'] = {
                'status': 'inherited'
            }
    ...
    if ok:
        ...
        if dim == [3, 3]:
            symmetry = construct_symmetry_dict(dim, spgo_init['number'],
                                               spgo_init['international'],
                                               spgo_init['pointgroup'])

        (prototype, labels,
         sitesort) = determine_wyckrep(pymg_init.as_dict(), spgo_init,
                                       symmetry)

        pymg_init.add_site_property('sorting_index', sitesort)
        pymg_conv = pymg_init.get_sorted_structure(
            key=lambda x: x.properties['sorting_index'])
        ...
        # node label set-up
        label = ' <> '.join([
            labels['symmetry_v'], labels['structure_v'], labels['chemistry'],
            labels['id']
        ])
        ...
        if 'a_conv' in outmode:
            ...
            if fmt == 'cif':
                astr_conv.set_extra('data_structure_original',
                                    data_structure_original)
        ...
        if 'a_prim' in outmode:
            ...
            astr_prim = StructureData(
                pymatgen=pymg_conv.get_primitive_structure(
                    tolerance=0.01).get_sorted_structure(
                        key=lambda x: x.properties['sorting_index']))

            if fmt == 'cif':
                astr_prim.set_extra('data_structure_original',
                                    data_structure_original)
        ...
        if 'a_primkkr' in outmode:
            ...
            (pymg_kkr,
             pymg_kkr_ok) = construct_pymg_kkr(pymg_conv, spgo_init, prototype)

            if pymg_kkr_ok:

                astr_prim_kkr = StructureData(
                    pymatgen=pymg_kkr.get_primitive_structure(
                        tolerance=0.01).get_sorted_structure(
                            key=lambda x: x.properties['sorting_index']))

                for (ikind,
                     kind) in enumerate(astr_prim_kkr.attributes['kinds']):
                    sumw = round(sum(kind['weights']), roundtom)
                    if sumw < 1.0:
                        astr_prim_kkr.attributes['kinds'][ikind][
                            'symbols'] += ('X', )
                        astr_prim_kkr.attributes['kinds'][ikind][
                            'weights'] += (round(1.0 - sumw, roundtom), )

                if fmt == 'cif':
                    astr_prim_kkr.set_extra('data_structure_original',
                                            data_structure_original)
